...
const fileMap = {
  ...
  [path.join(projectRoot, 'vistas', 'configuracion.html')]: 'header-index.html',
  [path.join(projectRoot, 'vistas', 'expirado.html')]: 'header-expirado.html'
};
function processFile(htmlPath, headerName) {
  if (!fs.existsSync(htmlPath)) {
    console.warn(`Skipped missing HTML entry: ${path.relative(projectRoot, htmlPath)}`);
    return;
  }
