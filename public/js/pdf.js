...
/* Clasifica un examen individual y devuelve un objeto con:
    ...
    */
    ...
    function renderTablaInline(rows) {
        var esSecrecionVaginal = rows.length > 0 && rows[0].esSecrecionVaginal;
        var html = '<table style="width: 100%; border-collapse: collapse; font-size: 0.78rem; margin-bottom: 10px; page-break-inside: auto;">';
        html += '<thead><tr><th style="font-weight: bold; padding: 4px 6px; border: 1px solid #000; background: #e9e9e9; width: 35%; text-align: left;">Examen</th>';
        if (esSecrecionVaginal) {
        ...
        rows.forEach(function(row) {
            if (esSecrecionVaginal) {
                html += '<tr><td style="font-weight: 600; padding: 4px 6px; border: 1px solid #ccc;">' + row.nombre + '</td><td class="' + row.clase + '" style="padding: 4px 6px; border: 1px solid #ccc; text-align: center;">' + row.texto + '</td></tr>';
            } else {
                html += '<tr><td style="font-weight: 600; padding: 4px 6px; border: 1px solid #ccc;">' + row.nombre + '</td>' +
    ...
    function buildInlineHtml(payload) {
        ...
        if (payload.heces) {
            ...
            html += '<h3 style="font-size: 0.85rem; font-weight: bold; text-align: center; text-transform: uppercase; border-bottom: 1px solid #000; padding-bottom: 2px; margin: 12px 0 8px;">Examen de Heces</h3>';
            html += '<table style="width: 100%; border-collapse: collapse; font-size: 0.78rem; margin-bottom: 10px; page-break-inside: auto;"><tbody>';
            function hecesRow(label, val) {
                html += '<tr><td style="font-weight: 600; padding: 4px 6px; border: 1px solid #ccc; width: 40%;">' + label + '</td><td style="padding: 4px 6px; border: 1px solid #ccc;">' + (val || '-') + '</td></tr>';
            }
            ...
            html += '<tr><td style="font-weight: 600; padding: 4px 6px; border: 1px solid #ccc; width: 40%;">Sustancias Reductoras</td><td style="padding: 4px 6px; border: 1px solid #ccc;">' + (d.sustanciasReductoras ? d.sustanciasReductoras + ' (' + window.interpretarSustanciasReductoras(d.sustanciasReductoras).texto + ')' : '-') + '</td></tr>';
        ...
        /* Uroanálisis */
        if (payload.uro) {
            html += '<h3 style="font-size: 0.85rem; font-weight: bold; text-align: center; text-transform: uppercase; border-bottom: 1px solid #000; padding-bottom: 2px; margin: 12px 0 8px;">Examen de Orina / Uroanálisis</h3>';
            payload.uro.ordenGrupos.forEach(function(grupo) {
                if (!payload.uro.grupos[grupo]) return;
                html += '<h4 style="font-size: 0.78rem; font-weight: 600; font-style: italic; margin: 4px 0 2px; text-align: center;">' + grupo + '</h4>';
                html += '<table style="width: 100%; border-collapse: collapse; font-size: 0.78rem; margin-bottom: 10px; page-break-inside: auto;"><tbody>';
                payload.uro.grupos[grupo].forEach(function(f) {
                    var val = payload.uro.datos[f.id] || '-';
                    if (val === '') val = '-';
                    html += '<tr><td style="font-weight: 600; padding: 4px 6px; border: 1px solid #ccc; width: 40%;">' + f.nombre + '</td><td style="padding: 4px 6px; border: 1px solid #ccc;">' + val + '</td></tr>';
                });
                html += '</tbody></table>';
            });
        }
